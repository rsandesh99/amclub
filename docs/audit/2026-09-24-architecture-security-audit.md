...
- **Status:** Fixed in code (wave 1). The hook verifies the Standard Webhooks signature and fails closed on production without `SEND_SMS_HOOK_SECRET`; +91 mobiles only, a per-phone limit, an 8 s MSG91 timeout. Needs the secret set in the Supabase "Send SMS" hook and in Vercel
