...
| M2 | Refunds, payouts and reconcile still go through the simulation gateway on production | Fixed (wave 5b, PR #64, ADR 027) |
| M3 | The agent token endpoint accepts delegated tokens: renewal forever, wider scopes, any run id | Fixed (wave 3, PR #61) |
| M4 | A goods RFQ sends the buyer's delivery contact (name, phone, address) to every matched seller before any order | Fixed (waves 4 + 5c, PRs #62 + #65); migration 0083 (applied 2026-09-24) |
...
| M10 | Coupons: every active code is publicly listable, usage limits are not atomic, no per-buyer limit | Fixed (waves 3 + 5b, PR #64, ADR 029); migration 0081 (applied 2026-09-24) |
| M11 | Key admin decisions are not audit-logged (provider approve / reject, coupon creation, CMS banners) | Fixed (wave 5b, PR #64) |
| M12 | Verification flags prove existence, not ownership (Udyam number, penny-drop name match) | Fixed (wave 5c, PR #65), ADR 028; migration 0082 (applied 2026-09-24) |
| M13 | Suspended providers keep their powers: quote, get paid, accept and deliver, read matched RFQs | Fixed (wave 3, PR #61) |
| M14 | A goods return can be opened from `completed` with no time limit | Fixed (wave 5c, PR #65) |
...
| M16 | Edits to approved Mart listings go live without re-review (category / commission, GST rate, images) | Fixed (wave 5c, PR #65); migration 0077 (applied 2026-09-24) |
...
| L4 | The pool "Pay ₹X to confirm" amount is computed on the client before GST (web and mobile) | Fixed (wave 5c, PR #65) |
...
| L9 | Two concurrent (or resumed) pool closes can release the quote slot the pool's own quote holds | Fixed (wave 5c, PR #65); migration 0077 (applied 2026-09-24) |
...
- **Status:** Fixed (wave 3, PR #61). The session path of `POST /api/v1/agent/token` refuses a delegated token (no self-renewal or scope widening), and `run_id` must be one of the caller's own runs (403 `run_not_yours`)
...
- **Status:** Fixed (waves 4 + 5c, PRs #62 + #65; 0083 applied 2026-09-24). API views strip the buyer's contact for sellers (wave 4); migration 0083 withdraws `rfqs.goods_spec` from client roles (column grant built from the catalogue) and the one session-client read (checkout quote branch) now uses the service role after the ownership check
...
- **Status:** Fixed (wave 4, PR #62). An RFQ may reference only the buyer's own uploads (`rfq-attachments/<msmeId>/<uuid>.<ext>`, else 422 `attachment_invalid`); the loaders sign only paths under the RFQ owner's prefix and pass legacy external links through only when https
...
- **Status:** Fixed (wave 3, PR #61). `requireAdmin` refuses every delegated agent token unless the route names its read-only ops tool (`summarize_dispute`, `triage_verification`, `recommend_payout_release`, `read_order_evidence`) and that tool is in scope. Admin mutations are never reachable by an agent token
...
- **Status:** Fixed (wave 3, PR #61). A report is recorded, not acted on; a review is hidden pending ops only after three distinct reporters (never counting the reviewed provider), and never again once ops restored it
...
- **Status:** Fixed (waves 3 + 5b, PR #64, ADR 029). No client read (0075); checkout claims a coupon use under the coupon row lock before any payment opens (`claim_coupon_for_session`, 0081): two buyers at the last use get one claim and one 409; optional `coupons.per_buyer_limit`
...
- **Status:** Fixed (wave 5b, PR #64). Verifications, coupons and CMS POST / PATCH write `audit_logs` with before / after, carry the adminMutation limit and `requireNotDelegated`; verifications decide only `pending_kyc` / `under_review` (409 `not_pending`, CAS); `reactivate` lifts only a suspension (409 `not_suspended`)
...
- **Status:** Fixed (wave 3, PR #61). `resolveActor` gives a suspended or soft-deleted provider no provider identity (like a suspended buyer) until an admin reactivates them, so quote, deliver, get-paid and matched-RFQ paths refuse
...
- **Status:** Fixed (wave 4, PR #62). The public pool API (list, detail, "mine") omits the agent rationale (`publicPool`), and migration 0076 withdraws the column from client roles
...
- **Status:** Fixed (wave 3, PR #61). Migration 0075 grants clients every `rfq_clarifications` column except `provider_id` and `answered_by`; the RLS policies that use them keep working
...
- **Status:** Fixed (wave 5b, PR #64, ADR 027). The webhook settles `refund.processed / failed`, `transfer.processed / failed / reversed` and `payment.dispute.created / lost / won / closed`; every handler writes only from the state it expects and records its event once per gateway id (replays change nothing); a failed or reversed transfer is never reused; an open chargeback holds the payout and blocks every release until Razorpay decides. A "re-send failed refund" admin action and a stuck-refund alert are open
...
- **Status:** Fixed (wave 5b, PR #64, ADR 029). 409 `self_dealing` on services and Mart checkout, group-buy join and checkout, a quote on your own request, order actions where one person owns both sides, and a review of your own provider; both fan-outs skip the buyer's own provider profile. Flagging shared bank accounts or phones across users is open
...
- **Status:** Fixed (waves 2 + 5b, PR #64). The captured-payment scan pages the whole window (wave 2); a second capture on a paid session is a `duplicate_capture` exception refunded in full, and reconcile counts only orders it created (wave 5b)
...
- **Status:** Fixed (wave 4, PR #62). Quote scope / message (every quote write goes through `quoteRowColumns`: submit, revision, pool close) and group-offer text are contact-masked with shared `redactContactInfo` before storage
...
- **Status:** Fixed (wave 5c, PR #65). The quote PATCH refuses a group quote (409 `pool_quote_fixed`); `verify-pools` proves the price stays at the achieved tier
...
- **Status:** Fixed (wave 5c, PR #65). A user who also owns a provider profile is left out of group detection, cannot join or choose an offer (403 `dual_role`) and sees no offers; the check fails closed
...
- **Status:** Fixed (wave 4, PR #62). A licence certificate is signed or deleted only when its stored path is under `<msme_id>/<licence_id>/`
...
- **Status:** Fixed (wave 5c, PR #65). The pool API returns `member.amounts` and `pool.unitDisplay` computed like checkout; web PoolJoin, My pools and the mobile pool screen render only those (client-money baseline −2)
...
- **Status:** Fixed (wave 5c, PR #65). A per-pool close lease (`close_lease_until`, compare-and-set, 10 min) and `quotes.pool_member_id` written at insert (0077): on a unique violation the close links its own quote and keeps the slot
