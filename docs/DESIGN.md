...
*Status (2026-09-24): **Group requests built dark (S3.4, ADR 024)** (`docs/agents/AGGREGATION.md`). Code proposes a group of buyers' OWN open requests for the same service in the same state. Each buyer opts in (one `ai_decisions` row). Fan-out-matched providers state ONE sealed volume-tier offer up front: the price for one business, then lower prices at higher counts. There is no counter-offer, no auction, and no provider sees another's offer. At close, each member who chose an offer gets ONE ordinary quote at the tier the count reached, and pays it through the ordinary checkout. There is no new order state or money path. Enablement: the V1.5→V2 gate + its §8.1 note (migration 0071 is applied to production, 2026-09-24).*
...
Experience v3 (`docs/prd/PRD_EXPERIENCE_V3.md`; each epic's Events table is canonical, listed here as it lands). E4 packages: `package_viewed { tiers }` · `tier_selected { tier }` · `compare_matrix_viewed` · `buy_now_clicked { tier, total_bucket, surface? }` · `refund_policy_opened` (client) · `package_group_saved { tiers, rows, created }` · `package_group_deleted` (server). E2 discovery: `search_performed { q_len, filters, sort, results, view }` · `search_filter_changed { filter, value }` · `search_feedback_given { helpful, reason }` · `search_zero_results` · `search_to_requirement_clicked { zero }` (client) · `search_feedback_stored { helpful, reason, results }` (server). E2b: `service_page_viewed { service }` · `shortlist_added { count }` · `compare_opened { n }` · `recent_view_opened { kind }` · `search_voice_used { lang, ok }` (client). E5 checkout: `checkout_viewed { logged_in }` · `checkout_inline_auth_started` · `checkout_inline_auth_completed { new_profile }` · `checkout_itc_shown` · `payment_initiated { coupon, gst_invoice }` (client; payment success stays the webhook's server event). E6 requirements: `requirement_form_started { entry }` · `requirement_voice_used { lang, ok }` · `requirement_strength_changed { bucket }` · `requirement_document_toggled { key, on }` · `requirement_must_have_set { kind, value?, on }` · `requirement_submitted { score, required_filled, entry }` · `requirement_entry_clicked { entry }` (client). E9 homes: `home_viewed { actions }` · `home_action_clicked { kind }` · `home_pickup_clicked { kind }` · `buy_again_clicked { price_changed }` · `requirement_repeated` (client; web + mobile). E9b (dark, D-PRD5): `licence_added { source }` (client + server) · `obligation_checklist_viewed { rules }` (client) · `renewal_reminder_sent { threshold }` (server). E10 provider onboarding: `onboarding_step_viewed { step, category }` · `onboarding_step_completed { step, ms }` · `onboarding_gstin_autofilled { fields }` · `onboarding_submitted { category, path }` (client) · `onboarding_abandoned { step }` · `onboarding_nudge_sent { step }` (server). E11a provider workspace: `partner_home_viewed { actions }` · `partner_action_clicked { kind }` · `inbox_filter_changed { key }` · `inbox_sorted { key }` (client). E11b quote form: `quote_form_started { entry }` · `quote_preview_shown { gst_mode }` · `quote_submitted { from_draft }` (client). E11c: `insights_viewed { range }` · `tender_alert_opened` · `tender_alert_feedback { useful }` · `gem_checklist_viewed` (client). E7 compare v3: `compare_viewed { quotes }` (client, the page view; the S1.2 server `compare_viewed` still fires from the pointers route) · `compare_sorted { key }` · `quote_shortlisted` · `quote_accepted` (client; the confirm tap — payment truth stays the webhook) · `quote_declined` (the existing S1.2 server event) · `quote_lost_labelled { rfq_id, n }` (server, from finalize). E8 order workspace: `order_viewed { status, role }` · `order_next_action_clicked { action }` · `order_tab_viewed { tab }` (client). E8b order messaging: `order_message_sent { redacted, docs }` · `order_message_read` (client). E13 mobile (every web event also carries `platform: 'android'` on mobile): `role_switched { to }` · `listing_status_changed { status }` · `invoice_opened` · `review_replied` · `availability_saved` · `deliverable_uploaded` (client); E13c: the E10 onboarding events fire from the native wizard too. E14 language: `locale_changed { from, to }` (client; web header + gateway, mobile settings). E14c (N32b, dark): `content_translation_drafted { lang, kind }` (client) · `content_translation_approved { lang, edited }` (client + server) · `translation_viewed_original` (client). E15 data foundations: `compare_flag_viewed { flag }` (client) · `shadow_predicted { feature, n }` · `shadow_resolved { rfq_id, n }` (server counters, distinct id `system`). E15b: `corpus_consent_changed { on }` (client; the server event adds `deleted`). E12a add-ons (ADR 019, dark): `addon_toggled { on }` (client, buy box) · `checkout_addons { count }` (client, at pay) · `package_addon_saved { created, active }` (client + server, provider editor) · `buy_now_clicked` gains `addons` (count) when any are chosen. E12b quote options (ADR 020, dark): `quote_option_added { label }` (client, provider form) · `quote_option_selected { label }` (client, compare). E12c bundles (ADR 021, dark): `bundle_viewed { milestones }` · `plan_cancel_requested { remaining }` (client) · `bundle_purchased` · `bundle_milestone_completed { seq }` · `plan_cancelled { cancelled, failed }` · `bundle_milestones_saved { n }` (server). E16a Mart storefront v2 (staged, Launch Gate): `search_mode_switched { to: services|goods, has_query }` (client; the Services | Goods switch, rendered only with `MART_ENABLED`). E16c: `mart_reorder_clicked { price_changed }` · `mart_reorder_reminder_set { on, interval_days }` (client, `/app/mart/reorder`).
WhatsApp consent + HELP menu (ADR-030 §2): `wa_consent_changed { purpose, optIn, source }` (server, `/me/whatsapp`; the runtime sends it for START / STOP with `purpose: transactional+assistant | all`, `known`, `recorded`, `personas`) · `wa_menu_shown { known, via: keyword|greeting|button|text|join, locale }` · `wa_menu_item { item, known }` · `wa_language_changed { from, to, via: keyword|button, known }` · `wa_dpdp_request { kind: access|erasure, known }` · `wa_report { known }` · `wa_rebind_prompted` (runtime; distinct id = the user, or `wa_conv:<conversation id>` for a number that is not an account — never a phone) · `support_ticket_opened` gains `via: wa_menu|wa_report` when the menu opens it.
WhatsApp user surfaces (PRD_WHATSAPP W1; client, web + mobile with `platform: 'android'`): `whatsapp_consent_changed { purpose, on, source: web_settings|mobile_settings }` · `whatsapp_optin_card_shown` / `_clicked` / `_dismissed { persona }` · `notification_settings_saved { field: channel|quiet_hours|pause|digest, persona? }` · `privacy_request_created { kind, persona? }` · `help_link_opened { target }` (mobile Help) · `locale_changed` gains `persisted` (web: also saved to the account). The signup / checkout WhatsApp opt-in is recorded by the consent API (`wa_consent_events`, source `signup` / `checkout`), not by analytics.
Notifications (ADR-030 §4): `notification_prefs_changed { whatsapp_off, sms_off, email_off (categories switched off), changed, quiet_hours, paused, digest }` (server, `PUT /me/notification-preferences`). Delivery itself is counted from `notification_outbox` (status, channel, fallback_of) and `wa_messages`, never from analytics.
WhatsApp + privacy ops (ADR-030 §6; server, distinct id = the admin): `wa_ops_reply { surface: unrouted, outcome, reason }` (a reply to a number no ticket holds) · `support_ops_reply { channel: thread|whatsapp, outcome, used_template }` · `dpdp_request_actioned { kind, source, action, overdue }` (`/admin/privacy`).
