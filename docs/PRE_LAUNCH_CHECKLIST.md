...
| order_message (Experience v3 E8b; `{{1}}` "New message on order #A1B2" — the order number only, never the message text; a party of the order, transactional; needed when `order_messaging_enabled` turns on) | `amc_order_message_en`, `amc_order_message_hi`, `amc_order_message_te` |
