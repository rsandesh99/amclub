...
| Razorpay live keys + webhook registration (production checkout is closed until then — ADR-023; simulate mode is off on production) | **PENDING** | ADR-003 cutover procedure (amended by ADR-023); `rzp_live_` guard in `lib/payments/index.ts` | Founder + engineering | Cutover date — FOUNDER_FILL |
