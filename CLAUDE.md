...
- **Agent programme (ADR-009).** Read `docs/agents/ARCHITECTURE.md` before any agent PR; build order and prompts in `docs/agents/BUILD_PROMPTS.md`. Runtime = `apps/agent-runtime` (Fly bom) + `packages/agent-core`; agents call `/api/v1` under a delegated token (session or **delegation grant**); `ai_decisions` is the confirmation ledger for every AI proposal (lifted to unstaged 0027 in S0.1), `agent_events` the trace; per-agent switches in `agent_settings` (closed registry) under the `AGENT_ENABLED` master; untrusted content only via `envelope()`; no admin tool except the rules-gated release in S4.1.
