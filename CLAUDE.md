...
- **S2.4 landed (dark):** AMC Score v1 — deterministic formula in shared `score.ts` (`SCORE_VERSION`, weights in code, 90-day window, sample gates, null = neutral prior), nightly `cron/score-compute` via `score_inputs_provider/buyer` functions into `provider_scores` / `buyer_scores` / `score_history` / append-only `score_events`; provider sees own score (`score_card_enabled`), buyers never see a number, admins see both; reliability-adjusted compare ordering server-side above `reliability_rank_threshold_paise` (`reliability_rank_enabled`); Munshi weekly growth nudge (template, no model); ADR-010; migration 0042. Runbook `docs/agents/SCORE.md`.
