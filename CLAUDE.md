...
- **Migration 0022 is STAGED** — it deploys with the `MART_ENABLED=true` release (Launch Gate §8.2). `verify-migrations.ts` marks it `staged`; use `MART_MIGRATIONS_EXPECTED=false` when verifying a database without them. **Prod status (2026-09-24): 0022–0025 and 0069 are applied** (Mart launches together with services). `MART_ENABLED` stays off until the founder's go, and the staged-column rule below still holds so the flag-off path stays inert and the kill switch stays clean.
