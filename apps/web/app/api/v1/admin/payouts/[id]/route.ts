...
/**
 * Phase 8 §7 — payout retry: failed|held → scheduled, validated against the
 * canonical PAYOUT_TRANSITIONS map (§2.5 rule 8) and audit-logged. The payout
 * batch cron picks the rescheduled row up on its next run.
 */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await writeAudit(admin, request, {
    ...
    entityId: id,
    before: { status: from },
    after: { status: 'scheduled', amount_paise: Number(payout.amount_paise) },
  })
