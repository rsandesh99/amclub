 ...
 *  cancellation refund that failed earlier is re-driven (ADR 026). A refund
 *  still failing marks the run degraded (M35). */
export async function GET(request: NextRequest) {
  ...
  return runCronJob(admin, 'auto-cancel', async () => {
    ...
    return { checked: stale.length, cancelled, refundsRedriven: refunds.refunded, refundsStillFailing: refunds.failed }
  })
}
