 ...
 *  order action could not (ADR 026). */
export async function GET(request: NextRequest) {
  ...
  const result = { ...captured, unconfirmedPayouts: payouts, missingInvoices: invoices }
  await recordHeartbeat(admin, 'reconcile', result as Record<string, unknown>)
