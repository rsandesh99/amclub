...
export default async function PoolOgImage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: '#0A2E22', color: '#FCFAF3', padding: 56, fontFamily: 'sans-serif', position: 'relative' }}>
        ...
        {pool ? (
          <div style={{ display: 'flex', gap: 40, marginTop: 28, flex: 1 }}>
            <div style={{ width: 300, height: 300, borderRadius: 16, background: '#143F33', display: 'flex', overflow: 'hidden' }}>
              {pool.imageUrl ? <img src={pool.imageUrl} alt="" width={300} height={300} style={{ objectFit: 'cover', width: 300, height: 300 }} /> : null}
            </div>
