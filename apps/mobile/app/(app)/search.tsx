...
export default function SearchScreen() {
  ...
  const run = useCallback(async (query: string, s: SearchParams['sort'], v: boolean) => {
    setLoading(true)
    const r = await searchCatalog({ query, sort: s, verifiedOnly: v, limit: 24 })
      ...
      <View className="border-b border-gray-200 bg-surface px-4 py-3 gap-3">
        <View className="flex-row items-center gap-2 rounded-xl border border-gray-200 px-3 py-1.5">
          ...
          <TextInput
            ...
            value={q}
            onChangeText={setQ}
            onSubmitEditing={() => run(q.trim(), sort, verifiedOnly)}
        ...
        <View className="flex-row gap-2">
          {(['rating', 'price_asc', 'price_desc'] as const).map((s) => (
            <TouchableOpacity
              key={s}
              onPress={() => { setSort(s); run(q.trim(), s, verifiedOnly) }}
          ...
          <TouchableOpacity
            onPress={() => { const v = !verifiedOnly; setVerifiedOnly(v); run(q.trim(), sort, v) }}
