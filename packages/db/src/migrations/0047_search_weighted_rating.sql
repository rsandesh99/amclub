-- E0 / U9 (PRD Experience v3) — "Top Rated" is never set, yet it led the
-- default ordering and rendered as a badge. The badge is gone from every
-- surface; here the sort stops keying on it, and "rating" becomes a
-- review-count-weighted average (5 phantom reviews at 4.0).
-- Same signature and RETURNS TABLE as 0007 (top_rated stays in the row for
-- API compatibility), so CREATE OR REPLACE is enough — no DROP.
-- Rollback: re-run the CREATE OR REPLACE from 0007.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  FROM base b
  ORDER BY
    -- Rating = a review-count-weighted (Bayesian) average: 5 phantom reviews at
    -- 4.0, so a 5.0 from one review no longer outranks 4.8 from 126.
    (CASE WHEN p_sort = 'rating' THEN
       (coalesce(b.avg_rating, 0) * coalesce(b.review_count, 0) + 4.0 * 5) / (coalesce(b.review_count, 0) + 5)
     END) DESC NULLS LAST,
    ...
    -- Tie-break without top_rated (never set; E0 / U9).
    (coalesce(b.avg_rating, 0) * coalesce(b.review_count, 0) + 4.0 * 5) / (coalesce(b.review_count, 0) + 5) DESC,
    b.review_count DESC NULLS LAST, b.package_id
