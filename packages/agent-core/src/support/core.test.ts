...
describe('runSupportTurn — the engine', () => {
  ...
  it('how_to → the topic template, no lookups beyond the reference lists, numbers from the copy only', async () => {
    ...
    // audit §7: the commission is per category, so the line names no rate (the provider sees it before listing / quoting)
    expect(res.reply.text).toContain('commission')
    expect(res.reply.text).not.toMatch(/\d\s*%/)
